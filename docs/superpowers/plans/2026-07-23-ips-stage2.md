...
> 이후 반영된 구조는 [코드 원리](../../code_explained.md)를 참고한다. 이 계획의 체크박스는
> 현재 완료 상태가 아니며, 최신 정책은 [핵심 정책 보완](../specs/2026-09-12-core-policy-clarification-design.md)을 따른다.
...
- **verdict 실패 로깅** ([packet_source.cpp](../../../src/capture/packet_source.cpp)) — `nfq_set_verdict` 반환값이 음수면 `LOG(ERROR)`.
- **stop 요청 유실 경합** ([packet_source.cpp](../../../src/capture/packet_source.cpp)) — `stop_requested_` 플래그 분리(loop가 running_을 재설정하지 않게).
- **페이로드 읽기 실패 로깅** ([packet_source.cpp](../../../src/capture/packet_source.cpp)) — `else`에 `LOG(WARNING)`.
