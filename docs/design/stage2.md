...
> 센서 처리율을 넘으면 NFQUEUE 큐가 포화될 수 있다. 현재 코드는 길이 10,240개를 요청하며
> 실패 시 경고 후 기본값으로 동작한다. `--queue-bypass`는 listener가 없을 때의 우회이지
> 큐 포화 대책이 아니다. 현재 `NFQA_CFG_F_FAIL_OPEN`을 설정하지 않으므로 포화 시 기본 DROP이다.
> 전송률·실제 큐 설정·커널 드롭을 기록해 Rule 차단과 과부하 손실을 구분한다.
> 공식 근거와 장애별 범위는 [핵심 정책 보완 6장](../superpowers/specs/2026-09-12-core-policy-clarification-design.md)을 따른다.
