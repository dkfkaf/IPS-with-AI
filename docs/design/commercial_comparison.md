...
> `본 프로젝트` 열은 현재 구현, `처리 방침`은 확장 목표를 구분한다. 최신 확장 범위는
> [확장 설계](../superpowers/specs/2026-09-01-suricata-inspired-expansion-design.md),
> 차단·관찰의 경계는 [핵심 정책 보완](../superpowers/specs/2026-09-12-core-policy-clarification-design.md)을 따른다.

## 1. 결론

본 프로젝트는 **단일 방어 호스트의 IPv4 inbound에서 Rule 기반 즉시 차단과 AI 기반 사후
탐지·판정 적용 이후 출발지 IP 수신 차단을 시연하는 연구용 프로토타입**이다. 새 연결과 기존
연결 모두 차단 대상이다. OUTPUT은 양방향 Flow 통계를
...
| AI 대응 | 제품별 정책에 따라 차단·격리·경보 | 이상 판정 적용 이후 동일 출발지 IP 수신 패킷 TTL 차단 | 코드 존재, VM 검증 별도 |
| IPv6 | IPv4/IPv6 정책과 검사 | IPv4만 구현 | IPv6 Rule·로그·차단 확장 예정 |
| 패킷·세션 처리 | 조각 재조립, TCP stream 재조립, 재전송·순서 변경 처리 | 기본 IP/TCP/UDP 헤더와 플로우 통계 | TCP 재조립 확장 예정, IP 단편은 별도 한계 |
| 지원 프로토콜 | ICMP, SCTP, GRE, QUIC/HTTP3 등 다수 | TCP/UDP 중심 | 범위 제외 |
| L7 식별 | HTTP/DNS/메일 등 프로토콜 디코더와 앱 식별 | 없음 | DNS부터 HTTP/1·TLS metadata 순서로 확장 예정 |
...
| 위협 인텔리전스 | IP·도메인·파일 평판과 IOC feed | 없음 | 로컬 IP·도메인·SHA-256 dataset 예정, 자동 feed 아님 |
...
| 오탐 처리 | monitor/block/quarantine, 예외·승인·정책 롤백 | 화이트리스트, TTL 자동 해제 | 코드 존재, L7 관찰 유지·차단 면제는 확장안 |
...
| 로그·분석 | 중앙 수집, SIEM 연동, 장기 보존·검색 | 로컬 로그·Qt 이벤트 표시 | 구조화 JSON Lines 출력 예정, 중앙 수집기 자체는 제외 |
| 경보 | 이메일·syslog·SOAR·티켓 연동, 중복 억제 | Qt 트레이·로그, 오류 경보 빈도 제한 | 외부 연동 제외 |
| 관리 보안 | 다중 관리자, RBAC, 감사 로그, API | 로컬 단일 관리자 데모 | 인증·감사 로그는 향후 과제 |
| 설정 운영 | 정책 버전·백업·롤백·중앙 배포 | `config.json` 시작 시 로드 | root Unix socket·검증 후 원자적 reload 예정 |
...
3. AI 이상 판정 적용 이후 같은 출발지 IP의 **수신 패킷**을 새 연결·기존 연결 모두 TTL 동안 차단한다.
4. 화이트리스트·TTL 만료·AI 장애·AI 대기열 포화에서 fail-open 정책이 문서대로 동작한다.
5. 탐지율·오탐률·처리량·지연·NFQUEUE 드롭 수를 실험 환경과 함께 기록한다.

## 4. 발표에서의 표현

- 적절한 표현: “Rule+AI 탐지와 판정 이후 출발지 IP 수신 차단을 구현한 연구용 프로토타입이며, 기능별 검증 결과와 확장 목표는 별도로 제시한다.” 실제 실험 증거 없이 ‘검증 완료’라고 표현하지 않는다.
