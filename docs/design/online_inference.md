...
`split_benign`은 정상 행을 난수로 섞어 나누므로 이 조건을 충족했다고 볼 수 없다. `ml.audit_dataset`은
입력·분할 지문과 동일 특징 중복을 기록하지만 날짜·세션 격리는 검증하지 않는다. 실제 CSV 점검과
그룹 분할 검증은 아직 남아 있다. 실험 시작 전에 목표 오탐률·공격별 탐지율·수신
