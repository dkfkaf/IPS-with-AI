...
def extract_features(df):
    ...
    valid_rows = ~feature_frame.isna().any(axis=1)
    return feature_frame[valid_rows].to_numpy(dtype=np.float64), labels[valid_rows]


def split_benign(feature_matrix, labels, ratios=(0.6, 0.2, 0.2), seed=42):
