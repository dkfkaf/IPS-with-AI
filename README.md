...
```bash
# 패킷 처리 관련
sudo apt install iptables libnetfilter-queue-dev libgoogle-glog-dev
...
```bash
# IPS가 INPUT·OUTPUT 규칙을 자동 등록하며 Ctrl+C 종료 시 제거
sudo ./ips
```

root 권한과 `/usr/sbin/iptables`가 필요합니다. 프로그램은 전용 체인 `IPS_WITH_AI`와
...
- [ ] **3단계** — AI 연동, 이상 플로우 알림 및 동일 출발지의 후속 플로우 차단
- [ ] AI 분석 엔진 (오토인코더)
- [ ] 관리 대시보드 (Qt)
- [ ] 가상환경 공격 시뮬레이션 검증
